...
const geoWorker = new Worker('geo-enrichment', async (job) => {
  ...
  try {
    ...
    // Set when the Places text-search fallback resolved the address to a
    // specific named place (mall, hotel, society). Precise enough to send
    // the preview directly instead of parking it in pending_geo_review —
    // otherwise the agent gets no reply at all (no preview, and no
    // "couldn't pin" message since lowConfidence is false).

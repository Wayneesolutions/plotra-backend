 ...
 * WayneRing is configured) full calling access regardless of plan. So
 * dashboard_access and calling_access default TRUE (not false, unlike
 * multi_agent_whatsapp's precedent in 20260821_04) — an unmigrated/legacy
 * plan should keep behaving exactly as it does today once the real gate
 * checks land, not silently lose access.
 *
 ...
 *
 * max_whatsapp_numbers is deliberately NOT added here — this PR sat open
 * long enough that migration 20260830_01_dedicated_whatsapp_numbers.js
 * added the same column independently in the meantime (also
 * notNullable().defaultTo(1), with growth/unlimited backfilled to 3/10)
 * and merged first. Re-checked against current main before rebasing this
 * branch rather than assumed — see the "Task 3" investigation this
 * migration was rebased under.
 *
 ...
 * something to resolve inside a schema migration. (The actual tier-plan-
 * row migration lives in PR #18, 20260825_04_tier_plans.js — a separate,
 * also-still-open PR, not this one.)
 */
